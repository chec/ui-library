...
<Preview>
  <Story name="Colors - Marketing">
    {{
      data: () => ({
        colors: ['primary-gradient', 'dark-blue', 'primary-blue', 'hologram', 'primary-gradient-2'],
      }),
      template: `
...
<Preview>
  <Story name="Grays (Product & Marketing)">
    {{
      data: () => ({
        colors: ['black'].concat([6,5,4,3,2,1].map(n => `gray-${n}00`)).concat(['white', 'gray-gradient', 'dark-gray-gradient']),
      }),
      template: `
        <div class="py-16 flex justify-center space-x-2">
          <div

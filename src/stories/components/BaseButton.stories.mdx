...
<Preview>
  <Story name="Defaults">
    {{
      ...
      props: {
        color: {
          default: select('Color', ['brand', 'primary', 'secondary', 'blue', 'green', 'red'], 'secondary'),
        },
        variant: {
      ...
      template: `
        <div class="py-16 flex justify-center">
          <base-button :color="color" :variant="variant" :disabled="disabled">
            {{text}}
          </base-button>
        </div>
      `,
  ...
  <Story name="States and sizes" decorators={[withKnobs()]}>
    {{
      computed: {
        possibilities() {
          return makeVariants({
            color: ['brand', 'primary', 'secondary', 'blue', 'green', 'red'],
